...
export class PairSet {
    ...
    add(source, target) {
        ...
        const targetSet = this.#sources.get(source)
        if (! targetSet.has(target)) this.#size++
    ...
    forEach(callback) {
        this.#sources.forEach((targetSet, source) => {
            targetSet.forEach(target => callback(source, target))
        })
    }
}
