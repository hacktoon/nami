...
var HeightMap = function(gridSize, heightRange, roughness, opts){
    var opts = _.assign({
            callback: function(){}
        }, opts),
        gridSize = gridSize + 1,
        grid = Grid.new(gridSize, gridSize, undefined);
    ...
    var diamond = function(grid, point, size, offset){
        ...
        setPointHeight(point, average + offset);
    };
    ...
    var setPointHeight = function(point, height){
        ...
        opts.callback(grid, point, height);
    };

    setPointHeight(Point.new(0, 0), heightRange.start);
    setPointHeight(Point.new(grid.width-1, 0), heightRange.start);
    setPointHeight(Point.new(0, grid.height-1), heightRange.start);
    setPointHeight(Point.new(grid.width-1, grid.height-1), heightRange.start);
