...
var getViewInput = function getViewInput() {
  var option = viewInput.options[viewInput.selectedIndex];
  return option.value;
};

var getTileSizeInput = function getTileSizeInput() {
  return Number(tilesizeInput.value);
};

var getRoughnessInput = function getRoughnessInput() {
  return Number(roughnessInput.value);
};

var getPlatesInput = function getPlatesInput() {
...
var getCanvasMousePoint = function getCanvasMousePoint(e, viewCanvas) {
...
viewCanvas.addEventListener('mousemove', function (e) {
  ...
  plate += Direction.getName(tile.plate.direction); // heat = currentWorld.heatMap.grid.get(point),
  ...
  infoText.innerHTML = position + terrain + plate; //infoText.innerHTML = position + height + terrain + heatText + rainText;
});
viewCanvas.addEventListener('mouseout', function () {
