...
var GridFill = (function() {
    var _GridFill = function(baseGrid, startPoint, newValue) {
        ...
        baseGrid.set(startPoint, newValue);

        this.grow = function() {
            var currentLayerPoints = _.last(this.layers);
            if (this.complete){ return }

            var newLayer = [];
            currentLayerPoints.forEach(function(point){
                var neighborPoints = self.growNeighbors(point);
                newLayer = _.concat(newLayer, neighborPoints);
            });
        ...
        this.growNeighbors = function(point) {
            var neighbors = [];
            Point.neighborHood(point, 'axials', function(neighborPoint) {
                if (self.isPointFilled(neighborPoint)) {
                    return;
                }
                self.points.push(neighborPoint);
                baseGrid.set(neighborPoint, newValue);
