...
var HeatMap = function () {
  var _Heat = function _Heat(size) {
    ...
    var buildZones = function buildZones(zone, code) {
    ...
    var fillColumn = function fillColumn(point, code) {
      var baseY = point.y - 1;
      self.grid.set(point, code);

      while (baseY >= 0) {
        point = new Point(point.x, baseY);
        if (self.grid.get(point) != defaultValue) break;
        self.grid.set(point, code);
        baseY--;
      }
    };
